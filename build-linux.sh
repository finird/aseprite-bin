...
if [ ! -d "skia-$SKIA_VERSION" ]; then
  ...
  curl -fL -o "Skia-Linux-Release-x64-libstdc++.zip" \
    "https://github.com/aseprite/skia/releases/download/$SKIA_VERSION/Skia-Linux-Release-x64-libstdc++.zip" || {
    echo "ERROR: failed to download Skia archive for $SKIA_VERSION"
    exit 1
  }
  unzip -q "Skia-Linux-Release-x64-libstdc++.zip"
  popd >/dev/null
fi
